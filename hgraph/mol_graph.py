...
class MolGraph(object):
    ...
    def label_tree(self):
        ...
        prev_sib = [[] for i in xrange(len(self.clusters))]
...
if __name__ == "__main__":
    ...
    for s in sys.stdin:#test_smiles:
        ...
        """
        graphs,tensors,orders = MolGraph.tensorize([hmol, hmol], None)
        for node, attr in graphs[0].nodes(data=True):
            print node, attr['inter_label'], attr['cluster']

        lens = [x.size(0) for x in tensors[0][:-1]]
        print torch.cat([torch.arange(lens[0]).unsqueeze(1).long(), tensors[0][0], tensors[0][2]], dim=-1)
        print torch.cat([torch.arange(lens[1]).unsqueeze(1).long(), tensors[0][1], tensors[0][3]], dim=-1)
        print tensors[0][4]

        print '------------------------------'
        lens = [x.size(0) for x in tensors[1][:-1]]
        print torch.cat([torch.arange(lens[0]).unsqueeze(1).long(), tensors[1][0], tensors[1][2]], dim=-1)
        print torch.cat([torch.arange(lens[1]).unsqueeze(1).long(), tensors[1][1], tensors[1][3]], dim=-1)
        print orders

        #draw_graph(hmol.mol_tree, 'mol_tree.png')
        #draw_graph(hmol.mol_graph, 'mol_graph.png')
        """
