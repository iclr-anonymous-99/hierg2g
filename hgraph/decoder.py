...
class HierMPNDecoder(nn.Module):
    ...
    def decode(self, src_mol_vecs, max_decode_step=200, beam=5):
        src_root_vecs, src_tree_vecs, src_graph_vecs = src_mol_vecs
        batch_size = len(src_root_vecs)

        tree_batch = IncTree(node_fdim=2, edge_fdim=3)
        graph_batch = IncGraph(self.avocab, node_fdim=self.hmpn.atom_size, edge_fdim=self.hmpn.atom_size + self.hmpn.bond_size)
        ...
        for t in xrange(max_decode_step):
            batch_list = [ bid for bid in xrange(batch_size) if len(stack[bid]) > 0 ]
            ...
            if len(expand_list) > 0:
                ...
                cls_scores, icls_scores = self.get_cls_score(src_tree_vecs, expand_idx, cur_mess, None)
            ...
            for i,bid in enumerate(expand_list):
                ...
                for kk in range(beam): #try until one is chemically valid
                    ...
                    if len(inter_cands) == 0:
                        ...
                        sorted_cands = sorted( zip(inter_cands, assm_scores), key = lambda x:x[1], reverse=True )

                    for inter_label,_ in sorted_cands:
                        inter_label = zip(inter_label, attach_points)
