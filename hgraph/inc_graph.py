...
class IncBase(object):
    ...
    def __init__(self, node_fdim, edge_fdim, init_num_nodes=1000, init_num_edges=1000, max_nb=12):
        self.max_nb = max_nb
        self.graph = nx.DiGraph()
        self.graph.add_node(0) #make sure node is 1 index
        self.edge_dict = {None : 0} #make sure edge is 1 index

        self.fnode = torch.zeros(init_num_nodes, node_fdim).long().cuda()
        self.fmess = self.fnode.new_zeros(init_num_edges, edge_fdim)
        self.agraph = self.fnode.new_zeros(init_num_nodes, max_nb)
        self.bgraph = self.fnode.new_zeros(init_num_edges, max_nb)
...
class IncTree(IncBase):
    ...
    def __init__(self, node_fdim, edge_fdim, init_num_nodes=1000, init_num_edges=1000, max_nb=12, max_nsub=20):
        super(IncTree, self).__init__(node_fdim, edge_fdim, init_num_nodes, init_num_edges, max_nb)
        self.cgraph = self.fnode.new_zeros(init_num_nodes, max_nsub)
...
class IncGraph(IncBase):
    ...
    def __init__(self, avocab, node_fdim, edge_fdim, init_num_nodes=1000, init_num_edges=1000, max_nb=10):
        super(IncGraph, self).__init__(node_fdim, edge_fdim, init_num_nodes, init_num_edges, max_nb)
        self.avocab = avocab
        self.mol = Chem.RWMol()
    ...
    def get_mol(self):
        ...
        for batch_idx, batch_atoms in self.batch.iteritems():
            ...
            mol_list[batch_idx] = Chem.MolToSmiles(mol) if mol is not None else None
        return mol_list
